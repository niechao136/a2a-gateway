...
title: 系统架构总览
description: 解释 a2a-gateway 的三重角色、核心组件关系（含模型注册表与 resolve_llm 构图路径），以及一次请求穿过 nginx、FastAPI 路由、LangGraph 与持久层的控制/数据流。
...
a2a-gateway 是「基于 LangGraph 的多 Agent 平台」（`pyproject.toml:4`），技术栈为 FastAPI + LangGraph（后端）、Next.js + MUI（前端）、PostgreSQL（配置 + Checkpointer 同库）、Alembic、sse-starlette、a2a-sdk、MCP、langchain-openai/anthropic（`pyproject.toml:7-30`）。README 为空，事实上的项目概述在 `TODO.md:23-30`。
...
3. **运维管控中心**：JWT 管理员在 `/admin` 维护 Agent 配置、A2A 端点/MCP/技能/**模型**四类注册表、连接器（飞书/Telegram/Slack）、语音代理与技能导入审核。路由聚合在 `main.py:87-94`（含 `routes/models.py` 的 `/api/admin/models*`）。
...
nginx (唯一入口 :10099)
 ...
 │                                              ├─ routes/{chat,admin,registry,models,a2a_server,speech,connectors}
 │                                              ├─ agent_factory ──► graph.build_graph ──► create_react_agent
 │                                              │        │                  │
 │                                              │        │                  ├─ resolve_llm(agent.model_snapshot)
 │                                              │        │                  │     ├─ 绑定快照 → build_llm_from_snapshot（openai/anthropic）
 │                                              │        │                  │     └─ NULL → build_llm（全局 LLM_* 回落）
 │                                              │        │                  ├─ tools (a2a_call/resume, MCP, skills, scripts)
 │                                              │        │                  └─ pre_model_hook (压缩/挂起/技能再注入)
 │                                              │        └─ AsyncPostgresSaver (checkpointer 单例)
 │                                              ├─ repository ──► PostgreSQL (11 应用表, 快照+id 列表)
 │                                              ├─ llm / llm_probe ──► ChatOpenAI/ChatAnthropic、连通性探针
...
| 编排 | `graph.py`、`agent_factory.py`、`llm.py` | 共享 ReAct 图、实例缓存 `(id, updated_at)`、`resolve_llm` 按快照/回落选 LLM、历史压缩与技能注入 |
| 工具 | `tools.py`、`mcp_client.py`、`skills.py`、`sandbox_client.py` | A2A/MCP/技能/脚本四类工具 |
| 集成 | `a2a_client.py`、`routes/a2a_server.py`、`connectors/`、`routes/speech.py`、`llm_probe.py` | 上游 A2A、对外 A2A、平台入站、ASR/TTS 代理、LLM 连通性探针 |
| 持久 | `models.py`、`repository.py`、`migrations.py` | 快照写时解析、目录表、启动迁移 |
| 管控 | `routes/registry.py`、`routes/models.py`、`routes/admin.py` | A2A/MCP/Skill/模型注册表 CRUD、Agent 发布与测试 |
...
2. `_resolve_agent`：仅允许 `PUBLISHED` Agent（`chat.py:62-71`）。
3. 身份 cookie 确保 + 会话目录 upsert（所有权不被劫持）。
4. `get_agent_instance`：缓存命中则复用图，否则从快照构图；图内 LLM 由 `resolve_llm(agent.model_snapshot)` 决定（`agent_factory.py:70-99`、`graph.py:318-319`）。
...
两条链路汇聚到同一 LangGraph 路径，共享 checkpointer 与 pending 存储；构图时的 LLM 选择同样走 `resolve_llm`。

### 管理端模型变更

`PUT /api/admin/models/{id}` 更新后：`refresh_agents_for_model` 把新快照刷回引用它的各 Agent（保留各自 temperature/max_tokens 覆盖），并对每个 Agent `invalidate_agent` 使图缓存失效——无需重启进程（`routes/models.py:101-105`、`repository.py:572-584`）。

## 状态与生命周期

- **配置态**：PostgreSQL 应用表；写时解析为 JSONB 快照，构图零 DB 依赖（见 [数据模型](/openwiki/architecture/data-model.md)）。
- **模型态**：`llm_models` 注册表 + Agent `model_id`/`model_snapshot` 单选绑定；NULL 回落全局 `LLM_*`（[LLM 模型管理](/openwiki/concepts/llm-model-management.md)）。
...
- **安全**：管理员 JWT、聊天身份 cookie、按 Agent API Key 三套凭据；A2A/MCP 出站共享认证方案；模型 `api_key` 明文入库、出参脱敏；连接器平台验签；技能导入 SSRF/zip 防护；沙箱双容器隔离（[安全页](/openwiki/concepts/security.md)）。
...
- **扩展缝**：注册表（A2A/MCP/Skill/模型）+ Agent 勾选/单选绑定；连接器适配器接口；技能审核后注入工具/提示；`SANDBOX_URL` 空即关脚本执行。
...
| 路由挂载 | `main.py:87-94`（chat/admin/registry/models/a2a_server/speech/connectors） |
...
- LLM 选择只经 `resolve_llm`：有 `model_snapshot` 走 provider 分支，否则回落 `Settings.llm_*`；绑定快照时非法 provider/缺 key 抛错、不静默回落。
- 同一 `thread_id` 全局唯一，跨聊天/连接器/A2A 恢复共用。
- nginx 是唯一对外面；backend/frontend/gate 不直接对公网。

## 代表性测试与延伸阅读

- 契约：`tests/test_chat_api.py`、`tests/test_a2a_server.py`；域逻辑：`tests/test_graph.py`、`tests/test_agent_factory.py`、`tests/test_bindings.py`、`tests/test_llm_build.py`。
- 专题：[聊天生命周期](/openwiki/workflows/chat-lifecycle.md)、[Agent 图构建](/openwiki/workflows/agent-graph.md)、[LLM 模型管理](/openwiki/concepts/llm-model-management.md)、[部署拓扑](/openwiki/architecture/deployment.md)、[A2A 服务端](/openwiki/integrations/a2a-server.md)。
